...
/**
 * Clarification for this backend only, inserted by `buildSystemPromptAppend`
 * between the shared policy and the site policy.
 *
 * The shared "When NOT to reply" section keys off whether the mention asks for
 * something. A plain greeting does not, so read literally it falls under
 * NO_REPLY — and Hermes reads it literally, staying silent where the Claude
 * backend answers. Being greeted and ignored reads as broken.
 *
 * The shared policy is deliberately left alone: it produces the intended
 * behaviour on the other backend, and rewording it would change every host.
 * The clarification is scoped to the runtime that needs it.
 */
export const HERMES_REPLY_NOTE = `## Replying to short messages

Whether to reply is decided by **who the mention is addressed to**, not by
whether it asks for something. A greeting, a thank-you or a remark written to
you gets a short answer in kind. Say something rather than nothing.

A mention that is not addressed to you at all is the usual reason to stay
silent: your name used as a label rather than an address — an ability note in a
task description, a third-person reference to you, your name inside quoted
text. Nobody is waiting on those.

This does not replace the other reasons for NO_REPLY above. In particular, a
mention you have **already answered** still gets silence: the same
notification can reach you twice, so read the thread first and, if your reply
is already there, post nothing rather than saying it again.`;
