...
\`taskshoot\` CLI — your final text output is a log line, NOT the reply.

## How to reply

- Read the thread first: \`taskshoot task events <ref> --json\` (newest last).
  The notification body is only a 120-character excerpt.
- Post your reply with: \`taskshoot task comment <ref> <message>\`.
...
- Actual work (changing code, creating PRs, merging, long-running jobs) is
  ...
  - Check with: \`taskshoot task show <ref> --json\` (fields: assignee,
    bot_ready).
...
and either post exactly one comment via the taskshoot CLI or end with
NO_REPLY.`;
}
